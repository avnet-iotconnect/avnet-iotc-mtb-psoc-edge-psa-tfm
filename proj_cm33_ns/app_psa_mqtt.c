...
void app_psa_mqtt_setup_huk(void)
{
    ...
    /* Note: tfm_ns_interface_init() is called in main.c before scheduler starts */
...
void app_psa_mqtt_setup_sdk_credentials(IotConnectClientConfig* config)
{
    ...
    if (key_id == PSA_KEY_ID_NULL) {
        printf("Error: PSA: Credentials not initialized! \n");
        return;
    }

    config->x509_config.device_cert = crt_pem_buffer;
    config->x509_config.device_key = (const char *)&key_id;
    config->x509_config.device_key_size = sizeof(psa_key_id_t);
}